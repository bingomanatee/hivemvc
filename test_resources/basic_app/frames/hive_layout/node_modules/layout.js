var Component = require('hive-component');
var path = require('path');
var index_path = path.join(__dirname, './../../../../../index');
var mvc = require(index_path); //require('hive-mvc');
var _DEBUG = false;
var _mixins = {

};

function _load(cb) {
	var ll = require('loaders/layout_loader')(this);
	ll.load(function () {
		cb();
	})
}

function _static(cb){
	var static = this.get_config('static');
	if (static) {
		this.static = mvc.Static({}, {
			map: static,
			root: path.resolve(this.get_config('root'), 'static')
		});
		this.static.init();
	}
	cb();
}

function _enlist(cb) {
	if (_DEBUG) {
		console.log('enlisting layout');
	}
	var root = this.get_config('root');
	if (!root) {
		if (_DEBUG) {
			console.log('attempting to enlist layout - no root');
		}
		root = this.component_id; // gotta have something...
	}
	this.$root = root;

	var existing_root = layout_model.get(root);
	if (existing_root) {
		throw new Error('redundant layout for root %s', root);
	}
	if (_DEBUG) console.log('putting a layout in the model');
	layout_model.put(this);
	cb();
}

function _emit(cb) {
	if (_DEBUG) {
		console.log('emitting layout %s', this.get_config('root'));
	}
	mvc.emit('layout', this);
	cb();
}

var layout_model = mvc.Model({_pk: 'name', name: '$layouts'});

function Layout(root) {
	if (_DEBUG) console.log('Layout: creating a layout for %s', root);
	return Component(_mixins, {
		root:       root,
		init_tasks: [
			_load,
			_enlist,
			_static,
			_emit
		]});
}

Layout.list = layout_model;

module.exports = Layout