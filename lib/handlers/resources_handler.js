...
module.exports = function (apiary, cb) {
	...
	var _mixins = {
		...
		respond: function (params) {
			...
			var res_loader = resources_loader(params.file_path, apiary);
			var l = params.gate.latch();
			if (_DEBUG) {
				console.log('starting to laod resources %s', params.file_path);
			}
			res_loader.load(function () {
				if (_DEBUG) {
					console.log('done loading resources for %s', params.file_path)
				}
				l();
			});
		}
