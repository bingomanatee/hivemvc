...
var _mixins = {
	...
	respond: function (params) {
		...
		var self = this;
		fs.readFile(params.file_path, 'utf8', function(err, config_str){
			try {
				debugger;
				var configs = JSON.parse(config_str);
				console.log('routes: %s', util.inspect(configs));
				self._config.setAll(configs);
			} catch(err){
				console.log('cannot parse %s', params.file_path);
			}
			l();
