...
module.exports = function (context, output_callback) {
	...
	var self = this;
	var pipe = self.pipe ? _.map(self.pipe, function(n){ return [_pipette, n]}) : _default_pipes;
	...
	async.waterfall(respond_wf, function(err, ctx, output){
		if (!ctx){
			ctx = context;
		}
		...
		output_callback(err, ctx, output);
	});
};