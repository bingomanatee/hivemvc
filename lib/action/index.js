...
function _load(cb) {
	...
	this.load(function () {
		clearTimeout(lto);
		if (_DEBUG) {
			console.log('done with loading action');
		}
		cb();
	});
}
...
module.exports = function (apiary, callback) {
	...
	function _emit(cb) {
		if (_DEBUG) {
			console.log('emitting action %s', this.get_config('root'));
		}
	...
	function _enlist(cb) {
		if (_DEBUG) {
			console.log('enlisting action');
		}
		var root = this.get_config('root');
		if (!root) {
			if (_DEBUG) {
				console.log('attempting to enlist action - no root');
			}
	...
	function Action(mixins, config) {
		if (_DEBUG) {
			console.log('creating action with config %s', config);
		}
